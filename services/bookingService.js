...
//   const services = allActiveBookings.map(
//     (booking) => ({
...
//       selectedDate:
//         booking.selections.selectedDate,

//       selectedSlot:
//         booking.selections.selectedSlot,

//       totalPrice:
//         booking.selections.totalPrice,
//     })
//   );
...
export const createBookingService = async (
  ...
  const services = allActiveBookings.map(
    (booking) => {
      ...
      // MULTI OPTIONS
      if (
        selections.selectedMultiOptions
      ) {
        Object.values(
          selections.selectedMultiOptions
        ).forEach((group) => {
      ...
      if (
        selections.selectedPriceOption
          ?.label
      ) {
        selectedPriceOptions.push({
          title:
